...
tidied_post_meal_data |>
  summarise(
    max_BMI = max(BMI),
    min_BMI = min(BMI)
  ) # in case of missing values, you would add a ", na.rm = TRUE" in the end off
```
...
tidied_post_meal_data |>
  ...
  ungroup() # Det kan være godt at slutte med "ungroup()", for at undgå at R fortsætter med at gruppere efterfølgende data
```
